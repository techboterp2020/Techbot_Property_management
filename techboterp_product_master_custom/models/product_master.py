...
class ProductInherit(models.Model):
    ...
    partition_room_id = fields.Many2one('product.template', string="Room", domain="[('is_room','=',True)]",tracking=True)
    ...
    bed_space_ids = fields.One2many('product.template', 'room_id', domain="[('is_bed_space','=',True)]",tracking=True, string='Bed Space Name')
    room_partition_ids = fields.One2many('product.template', 'partition_room_id', domain="[('is_partition','=',True)]", tracking=True, string='Partition details')
    ...
    def _check_bed_partition_sold(self):
        ...
        for rec in self.bed_space_ids:
            ...
            for object in obj:
                ...
                if object.order_id.rental_status in ['draft','cancel'] and not object.order_id.rental_status == 'returned':
                    if object.order_id.name not in quotations:
                        quotations.append(object.order_id.name)
                if object.order_id.rental_status in ['pickup', 'return'] and not object.order_id.rental_status == 'returned':
                    raise UserError(_("Please Don't Change Leasing Method, Currently The Bed space is Running and Confirmed sale order are %s",quotations))
            ...
            for object in obj:
                if object.order_id.rental_status in ['draft','cancel'] and not object.order_id.rental_status == 'returned':
                    if object.order_id.name not in quotations:
                        quotations.append(object.order_id.name)
                if object.order_id.rental_status in ['pickup', 'return'] and not object.order_id.rental_status == 'returned':
                    raise UserError(_("Please Don't Change Leasing Method, Currently the Partition is Running and Confirmed sale order are %s",quotations))
