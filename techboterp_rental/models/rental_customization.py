...
class RentalSaleOrderInherit(models.Model):
    ...
    def onchange_rentby(self):
        for rec in self:
            rec.room_ids = rec.order_line = rec.is_room = rec.is_bed_space = rec.is_partition = rec.is_bed_partition = False
            if rec.rent_by == 'room':
                rec.is_room = True
...
class SaleOrderLine(models.Model):
    ...
    def on_change_is_rental(self):
        ...
        for rec in self:
            ...
            # Method to Show Multiple Products Based On Multiple Rooms Selection
